    ...
    <div class="message_wrap">
      <div class="msg_people">
        <div class="chat_list" v-if="latestChatUsers.length">
          <div class="chat_person" v-for="(item, index) of latestChatUsers" :key="index"
            ...
            <div class="wrap-i">
              <span :class="item.has_read == 'y' || item.has_read == '' ? 'dot' : 'dot onlline'"></span>
              <div class="short">
        ...
        <div class="button_wrap" :class="latestChatUsers.length ? 'button_wrap' : 'button_wrap single_btn'">
          <button type="button" :class="latestChatUsers.length ? 'gray_bordered' : 'gray_bordered disabled'"
            @click="getReadAll" :disabled="latestChatUsers.length ? false : true">
            모두 읽음
          </button>
      ...
      <div class="chat_box">
        <div class="main_wrap">
          ...
          <div class="header" v-if="latestChatUsers.length || selectMsg || defaultUserList || chooseUserList">
              ...
              <div class="user_wrap">
                ...
                <div class="user_dropdown_wrap">
                  <div class="selected_user_name" v-if="defaultUserList">
                    ...
                    <p class="user-name" v-if="defaultUserList != ''">{{ defaultUserList }}</p>
                  </div>
                  <div class="select_user_list" v-if="chooseUserList">
                    <a @click="() => (showUserDropdown = !showUserDropdown)">사용자 선택</a>
                    <div class="dropdown_wrap" v-if="showUserDropdown" v-click-outside="onClickOutside">
                      ...
                      <ul class="search_dropdown">
                        <li v-for="(item, index) of userList" :key="index" @click="setUserList(item)">
                          {{ item.department }}<span v-if="item.department">_</span>{{ item.rank }}<span
                            v-if="item.rank">_</span>{{ item.name }}
                        </li>
                      </ul>
                    </div>
                  </div>
          ...
          <template v-if="showChat && isChatroomOpen">
            <div class="all_chat" @scroll="handleScroll" id="scroll_div">
              <div class="chat_wrap">
                <div class="chat">
                  <!-- <p>{{ uTimeStamp }}</p>
                  <p>{{ localMessages }}</p> -->
                  <div class="date" v-for="(hdate, index) of uTimeStamp" :key="index">
                    <div class="devider"></div>
                    <span>{{ hdate }}</span>

                    <div v-for="(item, index) of localMessages" :key="index">
                      <div v-if="hdate == dateformat(item.timestamp)"
                        :class="item.user_id != userId ? 'left_box' : 'right_box'">
                        <div class="text">
                          <div class="message">
                            <div :class="item.content_type != 'text' ? 'message_inner with_attach' : 'message_inner'">
                              <span class="attach" v-if="item.content_type != 'text'"
                                @click="fileDownload(item.attachments, item.original_file_name)">
                            ...
                            <div class="delete" v-if="item.has_read != 'y'">
                              <PopperWrapper arrow placement="right" v-if="item.user_id == userId">
            ...
            <div class="type_box">
              <div class="send_box_wrap" v-click-outside="inputOutside">
                <input type="text" @click="writeMsg" v-model="msgText" @keydown="sentMsg($event, msgText)" />
                <div class="send_box">
                  ...
                  <div class="send">
                    <button type="button" :disabled="isSend ? disabled : ''" :class="isSend ? 'active' : ''"
...
export default {
  ...
  created() {
    ...
    SocketioService.socket.on('acknowledgement', (data) => {
      ...
      console.log('condition-view===', data.room_id, this.currentRoomId, this.isChatroomOpen);
      ...
      this.commonService
        ...
        .then((res) => {
          if (res.status == 200) {
            ...
            if (this.latestChatUsers.length > 0 && this.chooseUserList == false && this.showChat == false) {
              this.selectMsg = true;
            }
