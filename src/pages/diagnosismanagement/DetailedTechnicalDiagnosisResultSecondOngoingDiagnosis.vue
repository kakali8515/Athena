  ...
  <div class="info_protection">
                  ...
                  <table class="min-w-full text-center">
                    <thead class="bg-gray-50">
                      <tr>
                        ...
                        >
                          <div class="sortable-cell">
                            결과 <img src="@/assets/icon/DownSquare.svg" />
                          </div>
                        </th>
                        <th
                          scope="col"
                      ...
                      >
                        ...
                        <td class="text-sm text-gray-900 font-light px-6 py-4">
                          <select
                            ...
                            :class="{
                              orange:
                                selectColor[`select-${item.id}`] === 'orange',
                              red: selectColor[`select-${item.id}`] === 'red',
                              default:
                                !selectColor[`select-${item.id}`] ||
                                selectColor[`select-${item.id}`] === 'default',
                            }"
                          >
                            <option value=""></option>
                            <option
                        ...
                        <td class="text-sm text-gray-900 font-light px-6 py-4">
                          <input
                            ...
                            :value="item.explain"
                            @change="
                              isUpdateExplain($event, item.id, item.asset_id)
                            "
                          />
                        </td>
                        <td class="text-sm text-gray-900 font-light px-6 py-4">
    ...
    <div class="button_wrap fixed-button-wrap flex justify-between">
      ...
      <div class="buttons flex justify-end">
        <button @click="save" :disabled="isSave == false" type="button">
          저장
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
          ...
          if (res.status == 200) {
            ...
            for (
              ...
              this.updatedResult.push({
                ...
                result: this.result,
                explain: res.data.technical_evaluation_settings[i].explain,
                note: this.note,
    ...
    isUpdateEvaluationResult(event, id, explain, asset_id) {
      const selectId = `select-${id}`; // Assuming `id` is a unique identifier for the current `select` element
      const colorMap = {
        manual: "orange",
        weak: "red",
        good: "default",
      };
      const color = colorMap[event.target.value];
      this.selectColor = {
        ...this.selectColor,
        [selectId]: color,
      };
    ...
    isUpdateExplain(event, id, asset_id) {
      if (this.updatedResult.length > 0) {
        let checkId = this.updatedResult.find(function (item) {
          return item.id == id && item.asset_id == asset_id;
        });
